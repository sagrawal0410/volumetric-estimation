...
try:
    ...
    raise ImportError(
        "Could not import tless_volume_benchmark. Run from the repo root "
        "(volumetric-estimation/) or set export PYTHONPATH=."
    ) from exc
...
def prepare_bop_stereo_rendered(
    ...
    Uses tless_volume_benchmark BOP I/O (T-LESS layout).
    """
    ...
    if not candidates:
        raise ValueError(
            f"No BOP candidates for object {object_id} in split {split}. "
            "Check that test/train images are extracted and scene_gt.json exists."
        )
