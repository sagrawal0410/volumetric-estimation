...
from volume_benchmark.common.geometry import backproject_depth_to_object
...
def _remove_statistical_outliers(
    ...
    try:
        ...
        if points.shape[0] > 1500:
            return points
        k = min(nb_neighbors, points.shape[0] - 1)
...
def _largest_dbscan_cluster(points: np.ndarray, eps: float, min_samples: int = 10) -> np.ndarray:
...
def estimate_volume_convex_hull(
    frames: Sequence[Frame],
    K: np.ndarray,
    depth_min: float = 0.05,
    depth_max: float = 5.0,
    voxel_downsample: float = 0.002,
) -> tuple[float, np.ndarray, trimesh.Trimesh]:
    """Low-level convex hull on fused points. Returns (volume_m3, points, hull_mesh)."""
    points = fuse_backprojected_points(frames, K, depth_min=depth_min, depth_max=depth_max)
    points = _voxel_downsample(points, voxel_downsample)
    points = _remove_statistical_outliers(points)
    points = _largest_dbscan_cluster(points, eps=3.0 * voxel_downsample)
    if points.shape[0] < 4:
        raise ValueError(f"Need at least 4 points for convex hull, got {points.shape[0]}")
    hull = trimesh.convex.convex_hull(points)
    volume = abs(float(hull.volume))
    if volume <= 0 or not np.isfinite(volume):
        raise ValueError("Convex hull volume is non-positive or non-finite")
    return volume, points, hull
...
def estimate_convex_hull_volume(
    ...
    """
    Estimate object volume via fused depth convex hull.

    Writes ``fused_points.ply``, ``hull_mesh.ply``, and ``report.json`` under
    ``outputs/convex_hull/`` (or ``output_dir``).
    """
    scan = load_scan_or_raise(scan_dir)
    out = resolve_output_dir(scan.scan_dir, "convex_hull", output_dir)
    out.mkdir(parents=True, exist_ok=True)

    volume_m3, points, hull = estimate_volume_convex_hull(
        scan.frames,
        scan.K,
        depth_min=depth_min,
        depth_max=depth_max,
    ...
    trimesh.PointCloud(points).export(out / "fused_points.ply")
    ...
    report: dict[str, Any] = {
        ...
        "num_hull_vertices": int(len(hull.vertices)),
        "voxel_downsample": voxel_downsample,
        "expected_bias": "usually overestimates non-convex objects",
        "outputs": {
            "fused_points": str(out / "fused_points.ply"),
